...
| `test/ui.test.js` | 전체 정적 버튼 연결, 화면 이동, CRUD, 백업, 카메라 예외 경로, 캘린더와 Google 무료 최소 동기화 화면 테스트 |
...
- 파이프라인의 항목을 열면 `고객 담당자 직접 입력` 바로 아래에서 `고객 담당자 연락처 직접 입력`을 사용할 수 있습니다. 등록된 연락처를 선택하지 않은 고객도 이름과 전화번호를 함께 저장할 수 있습니다.
