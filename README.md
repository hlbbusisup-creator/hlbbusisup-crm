...
- 목록에서 고른 담당자는 `고객 담당자 연락처` 아래 `선택한 담당자`에 `이름 · 소속/직함 · 연락처`로 쌓이며 **여러 명을 고를 수 있습니다.** 이미 고른 사람은 목록에서 빠지고, 각 줄 오른쪽 `×`로 선택을 해제합니다. 맨 위 `대표` 담당자의 직함·연락처·이메일이 위 입력칸에 반영되며, 대표를 해제하면 다음 담당자가 대표가 되면서 연락처도 함께 바뀝니다.
