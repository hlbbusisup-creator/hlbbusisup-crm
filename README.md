...
6. 사용자를 한 명이라도 만들면 그때부터 저장할 때 로그인 화면이 뜹니다. 로그인 화면의 목록에는 **이름만** 나오고, 권한은 로그인한 뒤 오른쪽 위 이름 칸과 `내 계정` 창에서 확인합니다.
...
**사용자 전환** — 오른쪽 위 이름을 누르면 다른 사용자로 바꿉니다. 이때 화면을 처음부터 다시 열어 앞사람의 **관리자 인증, 불러온 사용자 목록·변경 이력, 편집 중이던 내용, AI 대화**가 넘어가지 않게 합니다. 저장하지 않은 변경이 있으면 먼저 확인을 받습니다.

> 단계·그룹·분류 같은 팀 공용 설정과 초보 모드·대시보드 순서·페이지 줄 수 같은 사용 환경은 작업공간에 공유됩니다. 모든 사용자가 같은 값을 보며, 사람마다 다르게 두려면 별도 작업이 필요합니다.

## 5-2. 변경 이력과 휴지통의 조회 범위

`설정 > 변경 이력`은 `crm_audit_log`에 쌓인 기록을 백업 파일 없이 화면에서 바로 봅니다. **보이는 범위는 로그인한 사람에 따라 달라집니다.**

| 상태 | 변경 이력 | 휴지통 |
|---|---|---|
| 로그인한 사용자 | **자기가 남긴 기록만** | **자기가 지운 항목만** |
| 관리자 인증(관리자 코드 입력) | 전체 · 사용자별로 좁혀 보기 | 전체 · 누가 지웠는지 함께 표시 |
| 사용자 계정을 쓰지 않는 작업공간 | 관리자 인증 필요(예전과 동일) | 전체(예전과 동일) |

- 범위 제한은 화면에서만 거르는 것이 아니라 **서버가 조회 자체를 사용자 기록으로 고정**합니다. 화면·사용자 목록도 그 범위 안에서만 만들어져 다른 사람 이름이 보이지 않습니다.
- **조건**: 대상 이름·요약·사용자 검색, 화면별, 동작별(입력·수정·삭제·복원), 기간(7·30·90일·전체), 사용자별(관리자 인증 시)
- **상세**: `상세`를 누르면 항목별 **이전 값 → 이후 값**을 표로 봅니다
- 휴지통 `비우기`는 **지금 보이는 항목만** 비웁니다. 일반 사용자가 다른 사람이 지운 항목까지 지우는 일은 없습니다.
- 누가 지웠는지 기록이 없는 예전 항목은 복구할 길이 막히지 않도록 모두에게 보입니다.

변경 로그는 기본 90일 보관합니다(`CRM_AUDIT_RETENTION_DAYS`). 더 오래 보관하려면 값을 늘리거나 정기적으로 백업을 내려받아 두세요.

## 5-3. 내 계정 · 비밀번호 변경 · 로그아웃

오른쪽 위의 이름(예: `강민지 편집자`)을 누르면 **내 계정** 창이 열립니다.

- **비밀번호 변경** — 기존 비밀번호와 새 비밀번호(6자 이상)를 입력해 직접 바꿉니다. 관리자 코드는 필요 없고, 자기 계정만 바꿀 수 있습니다. 기존 비밀번호는 **서버에서** 확인하며, 틀리면 바뀌지 않습니다. 비밀번호를 정한 적이 없으면 기존 칸을 비워 두고 새로 정합니다.
- **로그아웃** — 로그인 정보를 지우고 **처음 주소로 접속한 것과 같은 상태**로 돌아갑니다. 보던 화면 주소(#해시)까지 지워지고 사용자 로그인 화면부터 다시 시작하므로, 앞사람의 관리자 인증·편집 내용·AI 대화가 다음 사람에게 넘어가지 않습니다.

## 5-4. 활동 · 전체 검색 · 중복 정리 · 파이프라인 Excel
...
- 상단 바에는 로고 · 메뉴 · 오른쪽 도구(외부 DB 상태, 전체 검색, 사용자 이름, 사용 안내)만 둡니다. 로고 옆 `CRM` 글자와 오늘 날짜 표시는 메뉴 자리를 확보하기 위해 두지 않습니다.
